...
#define START_SECTOR_FIRMWARE	FLASH_SECTOR_5
#define SIZE_FIRMWARE			5
#define START_OFFCET_FIRMWARE 	0x00020000
...
class STM32IYModemPlatform final : public kocherga_ymodem::IYModemPlatform
{
public:
    virtual ~STM32IYModemPlatform() = default;
    ...
    Result emit(std::uint8_t byte, std::chrono::microseconds timeout) final {
    	HAL_UART_Transmit(&huart1, &byte, 1, timeout.count());
    ...
    Result receive(std::uint8_t& out_byte, std::chrono::microseconds timeout) final {
    	HAL_UART_Receive(&huart1, &out_byte, 1, timeout.count());
    	return Result::Success;
    }
     ...
     */
    virtual std::chrono::microseconds getMonotonicUptime() {
...
class STM32IROMBackend final : public kocherga::IROMBackend
{
private:
    uint32_t counter_offset = START_OFFCET_FIRMWARE;
public:
